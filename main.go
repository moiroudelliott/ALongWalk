...
func main() {
	...
	var filePath string
	fmt.Print("Veuillez entrer le chemin du fichier : ")
	fmt.Scanln(&filePath)
	...
	fmt.Printf("Plus long chemin trouvé : %d étapes\n", maxTotal)
	fmt.Println("\n===== Temps d'exécution =====")
	fmt.Printf("Temps de construction du graphe : %v\n", durationGraphBuild)
	fmt.Printf("Temps de parcours (DFS+pruning) : %v\n", durationDFS)
